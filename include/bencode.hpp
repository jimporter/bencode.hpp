...
#define BENCODE_DATA_GETTER(func, impl, arg_type, container_type)             \
  ...
  using basic_data_for_char_t = basic_data<
    Variant, long long, std::basic_string<Char>, std::vector, map_proxy
  >;

  template<template<typename ...> typename Variant, typename Char>
  using basic_data_view_for_char_t = basic_data<
    Variant, long long, std::basic_string_view<Char>, std::vector, map_proxy
  >;
  ...
  namespace detail {
    ...
    decode_digits(Iter &begin, Iter end, [[maybe_unused]] Integer sgn = 1) {
      ...
      for(int i = 0; i != std::numeric_limits<Integer>::digits10; i++) {
        ...
        if constexpr(std::is_signed_v<Integer>)
          value = value * 10 + (static_cast<char>(*begin++) - '0') * sgn;
        else
          value = value * 10 + (static_cast<char>(*begin++) - '0');
      }
      if(begin == end)
        throw end_of_input_error();
      ...
      if(std::isdigit(static_cast<char>(*begin))) {
        Integer digit;
        if constexpr(std::is_signed_v<Integer>) {
          digit = (static_cast<char>(*begin++) - '0') * sgn;
          check_over_underflow(value, digit, sgn);
        } else {
          digit = (static_cast<char>(*begin++) - '0');
          check_overflow(value, digit);
        }
    ...
    Integer decode_int(Iter &begin, Iter end) {
      assert(static_cast<char>(*begin) == 'i');
      ++begin;
      Integer sgn = 1;
    ...
    template<typename Char>
    class str_reader<std::basic_string_view<Char>> {
      using string_type = std::basic_string_view<Char>;
    public:
      template<typename Iter, typename Size>
      string_type operator ()(Iter &begin, Iter end, Size len) {
        if(std::distance(begin, end) < static_cast<std::ptrdiff_t>(len)) {
          begin = end;
          throw end_of_input_error();
        }

        string_type value(&*begin, len);
        std::advance(begin, len);
        return value;
      }
    };
      ...
      try {
        do {
          ...
          if(static_cast<char>(*begin) == 'e') {
            ...
            if(static_cast<char>(*begin) == 'i') {
              store(detail::decode_int<integer>(begin, end));
            } else if(static_cast<char>(*begin) == 'l') {
              ++begin;
              state.push(store( list{} ));
            } else if(static_cast<char>(*begin) == 'd') {
              ++begin;
